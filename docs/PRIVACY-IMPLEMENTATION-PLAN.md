...
| 8 | Note plaintexts are one fixed size (198 bytes); output order is randomised. Relayed transfers pay a flat per-asset fee (plus a share only on amounts leaving the pool), so the public fee carries nothing about a transfer's size |
